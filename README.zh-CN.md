...
按服务商设置订阅档位（预设或自定义月费）。`Quota Status` 标题行显示合并后的月度总费用，过期条目自动清理。

### ⚙️ 设置

| 选项卡 | 内容 |
|--------|------|
| **General（通用）** | 自动刷新周期（1 分钟 – 1 小时）、预测周期（7 / 14 / 21 天）、应用语言（跟随系统 / EN / 中文）、登录时启动、紧急徽章、CLI 安装、偏原生的主操作按钮 |
...
| **Advanced Providers（高级 Provider）** | ChatGPT 账号选择、状态栏窗口模式（5h / weekly / 同时）、自适应宽度的原生弹出按钮 |
| **Subscriptions（订阅）** | 使用原生 macOS 菜单选择器为配额制服务商设置预设套餐或自定义月费，使用更偏原生的 Apply 按钮样式，并显示各 Provider 图标以便快速识别 |
